...
class AIGenerator:
    ...
    def _upload_file_google_studio(self, page, profile_name, file_path):
        if not os.path.exists(file_path):
            return False
        try:
            uploaded = False
            
            # Try setting files directly first
            for inp_sel in ['input[type="file"][data-test-upload-file-input]', 'input[type="file"].file-input', 'input[type="file"]']:
                try:
                    ...
                    if inp_loc.count() > 0:
                        inp_loc.set_input_files(file_path, timeout=3000)
                        time.sleep(2)
            ...
            if not uploaded:
                for inp_sel in ['input[type="file"][data-test-upload-file-input]', 'input[type="file"].file-input', 'input[type="file"]']:
                    try:
                        inp_loc = page.locator(inp_sel).first
                        if inp_loc.count() > 0:
                            inp_loc.set_input_files(file_path, timeout=5000)
                            uploaded = True
                            break
                    except:
                        pass
                
            # 3. Fallback: try File Chooser
            if not uploaded:
                upload_menu = None
            ...
            print(f"[{profile_name}] Lỗi trong _upload_file_google_studio: {e}")
            return False
            ...
            for current_source in sources_to_try:
                try:
                    ...
                    if current_source == "chatgpt":
                        ...
                        if prompt_img and prompt_img.strip() and os.path.exists(input_image_path):
                            try:
                        ...
                        # --- PHẦN 2: XỬ LÝ TEXT ---
                        if prompt_img and prompt_img.strip():
                            print(f"[{profile_name}] Đang chuyển sang chat mới sạch sẽ để viết status...")
                            try:
                                page.goto("https://chatgpt.com/", wait_until="commit", timeout=60000)
                                page.wait_for_timeout(3000)
                            except Exception as e:
                                print(f"[{profile_name}] Warning: Không thể mở chat mới: {e}")

                        # Upload anh lam ngu canh cho viet status:
                        # CHI upload neu co prompt_img (tuc la da ve anh moi truoc do)
                        # KHONG upload neu: bai la video hoac text-only (tranh lam roi ChatGPT)
                        if prompt_img and prompt_img.strip():
                            text_image_path = output_image_path if os.path.exists(output_image_path) else input_image_path
                            if os.path.exists(text_image_path):
                                try:
                                    page.locator('input[type="file"][multiple]').first.set_input_files(text_image_path)
                                    print(f"[{profile_name}] Da upload anh lam ngu canh cho Status.")
                                    time.sleep(2)
                                except Exception as e:
                                    print(f"[{profile_name}] Loi upload anh ngu canh ChatGPT: {e}")

                        try:
                            # Chi dung prompt, KHONG kem status_base (da co trong prompt roi)
                            full_prompt = prompt
                        ...
                        # --- PHẦN 1: XỬ LÝ ẢNH ---
                        if prompt_img and prompt_img.strip() and os.path.exists(input_image_path):
                            print(f"[{profile_name}] Đang upload ảnh lên Google AI Studio để vẽ lại...")
                            success_upload = self._upload_file_google_studio(page, profile_name, input_image_path)
                            if not success_upload:
                                raise Exception("Không thể upload ảnh lên Google AI Studio.")
                            time.sleep(3)
                        ...
                        if prompt_img and prompt_img.strip():
                            print(f"[{profile_name}] Đang chuyển sang chat mới sạch sẽ để viết status...")
                            try:
                                page.goto(self.ai_studio_url, wait_until="commit", timeout=60000)
                        ...
                        # Upload anh lam ngu canh cho viet status:
                        # CHI upload neu co prompt_img (tuc la da ve anh moi truoc do)
                        # KHONG upload neu: bai la video hoac text-only
                        if prompt_img and prompt_img.strip():
                            text_image_path = output_image_path if os.path.exists(output_image_path) else input_image_path
                            if os.path.exists(text_image_path):
                                print(f"[{profile_name}] Dang upload anh lam ngu canh cho Status...")
                                self._upload_file_google_studio(page, profile_name, text_image_path)
                                time.sleep(3)
