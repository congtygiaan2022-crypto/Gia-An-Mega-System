        ...
        with sync_playwright() as p:
                ...
                for insta_url in instagram_urls:
                    ...
                    for post in posts:
                        ...
                        if is_proc:
                            ...
                            count_added += 1
                            # Nếu là đăng bài thật (không phải chỉ quét), giới hạn 3 bài/vòng để tránh spam
            ...
            if threads_urls:
                ...
                for threads_url in threads_urls:
                    ...
                    if not threads_url:
                        continue
                    # Tự động sửa sai threads.com thành threads.net
            ...
            for item in new_posts_found:
                ...
                if only_scrape:
                    ...
                    db_manager.set_status(profile_name, "Rewriting via AI")
                    try:
                        ...
                        p_log(profile_name, f"[{profile_name}] Loi xu ly AI: {ai_err}")
                        if temp_media_path and os.path.exists(temp_media_path):
                            try: os.remove(temp_media_path)
                            except: pass
                        raise ai_err
...
if __name__ == "__main__":
    try:
        if len(sys.argv) > 2:
            ...
            print("Usage: python worker_process.py <Profile_Name> <Current_Loop>")
            sys.exit(1)
    except Exception as e:
        import traceback
        print("\n" + "="*50)
