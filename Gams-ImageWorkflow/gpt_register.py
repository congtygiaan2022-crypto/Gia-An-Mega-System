        ...
        with sync_playwright() as p:
            ...
            for attempt in range(10):
                ...
                try:
                    ...
                    pass
                if not username:
                    # Fallback tạo username ngẫu nhiên nếu nút random không sinh
                    try:
                        rand_user = "usr" + "".join(random.choices(string.ascii_lowercase + string.digits, k=8))
                        page_mail.locator('#credentials').fill(rand_user)
                        username = rand_user
                    except Exception:
                        pass
                if username:
            ...
            acc_dir = os.path.join(config.get("profiles_dir", "profiles"), profile_name)
